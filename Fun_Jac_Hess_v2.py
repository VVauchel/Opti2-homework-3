...
def New_Obj(x, mu, lambd, A, B):
    ...
    # Evaluate Objective
    val = (p + np.mean(s) + np.mean(t)) / mu + Bar
...
def Hessian(x, lambd, A, B):
    ...
    for j in range(n):
        ...
        Hhh = [sum(A[i, j] * A[i, k] / Q1[i] for i in range(n_a))
               ...
               + (j == k) * 2 * lambd / Q3 + 4 * (lambd**2) * h[j] * h[k] / (Q3 ** 2)
               for k in range(n)]
        ...
        # dF/dh_j ds_k
        Hhs = [- A[k, j] / Q1[k] for k in range(n_a)]

        # dF/dh_j dt_k
        Hht = [B[k, j] / Q2[k] for k in range(n_b)]
    ...
    # dF/dp ds_k
    Hps = [0 for k in range(n_a)]

    # dF/dp dt_k
    Hpt = [0 for k in range(n_b)]
